...
[![lint](https://github.com/TeTeHacko/chytra-budka/actions/workflows/lint.yml/badge.svg)](https://github.com/TeTeHacko/chytra-budka/actions/workflows/lint.yml)
...
> The full MPPT + battery power path is designed and documented — measured
> power reality in [firmware/POWER.md](firmware/POWER.md), the charger design in
> [firmware/hw/POWER_UNIVERSAL.md](firmware/hw/POWER_UNIVERSAL.md) — but no
> autonomous solar node is deployed yet.
...
2. **A bench board** (~€28, USB-powered) — the full firmware on a breadboard, against any MQTT broker you already have (Home Assistant discovery included); relay/OTA/HTTPS-enrollment all optional.
...
- **Solar-ready power design** — MPPT charge path + 2× 18650 designed (see the
  power docs above), fuel-gauge SOC + hybrid power-mode firmware already in
  place; fielded units currently run on USB/powerbank.
...
board against it — the UI above comes alive with zero hardware and zero
toolchain:
...
```
(power chain below = TARGET topology; as-built today: USB/powerbank → 5V IN)

[Solar 18V/10W]
       │
       ▼
[WaveShare Solar Power Manager + 2× 18650]
       │  5V USB-C OUT
       ▼
[XIAO ESP32-S3 Sense] ── OV3660 camera (detachable; AliExpress variant ships OV3660)
       ...
       ├── MAX17048 I²C fuel gauge (true SOC %, addr 0x36)
...
I²C MAX17048 fuel gauge). The thresholds come from the
[power budget](DEPLOYMENT.md#power-budget-summer-tree-shade):

| Mode                | Trigger                                   | Power       | What it does                                                     |
| ------------------- | ----------------------------------------- | ----------- | ---------------------------------------------------------------- |
| **Continuous**      | enter at SOC ≥ 65 %, leave below 50 %      | ~150 mA avg | Continuous audio stream + Video-on-Demand camera trigger    |
| **Sound-triggered** | the default; holds between 30 % and 65 %   | ~12 mA avg  | VAD detector listens; on threshold → 30 s WiFi burst stream |
| **Safe**            | enter below 30 %, recover at ≥ 35 %        | ~12 mA avg  | Audio off, WiFi power-save; recovers to Sound-triggered     |

Realistic split in tree shade: ~25–30 % continuous, ~70 % sound-triggered. The
dawn chorus (4–9 AM) is prioritised for continuous mode when the battery allows.

## Hardware

XIAO ESP32-S3 Sense (onboard camera + PDM mic + microSD) + a handful of I²C
sensors in an IP65 box; the WaveShare Solar Power Manager + 2× 18650 chain is
the designed (not yet fielded) power stage. Full details:
...
- **[SCHEMATIC.md](SCHEMATIC.md)** — block diagram, power path, pin assignment
- **[firmware/hw/](firmware/hw/)** — KiCad project + carrier-PCB design
- **[DEPLOYMENT.md](DEPLOYMENT.md)** — site, climate, seasonal power strategy, energy budget
...
| [`SCHEMATIC.md`](SCHEMATIC.md) | Block diagram, wiring detail, power path, firmware logic |
| [`DEPLOYMENT.md`](DEPLOYMENT.md) | Site, climate, seasonal strategy, power/energy budget |
...
in an off-the-shelf IP65 box. The earlier-revision rationale lives in
[SHOPPING.md](SHOPPING.md), recent changes in [CHANGELOG.md](CHANGELOG.md).
