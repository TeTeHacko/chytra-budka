...
#
# `idf.py build` ALONE NEVER WORKS in this project: the flash size (8 MB),
# secure-boot/signing posture, and bench/field tweaks all live in sdkconfig
# overlay files, and IDF only applies them via a `-DSDKCONFIG_DEFAULTS=…;…;…`
# chain at set-target time. That chain used to live only in the docs (and in
# people's heads), so a plain `idf.py build` silently fell back to the 2 MB
# default and died at the partition table. This wrapper encodes every profile
# so nobody has to retype (or misremember) the chain.
#
# Usage:
...
# documented secure-boot path (SIGNING_CUSTODY.md) — build manually via
...
case "$PROFILE" in
  ...
  *)
    ...
    echo " not wired here — build them manually via idf.py -DSDKCONFIG_DEFAULTS=…; see SIGNING_CUSTODY.md.)" >&2
    exit 2
    ;;
esac
...
# config, like a stray `idf.py build` that defaulted to 2 MB flash) auto-heals.
