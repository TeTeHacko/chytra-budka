...
static const cfg_entry_t SCHEMA[] = {
     ...
     * reconnect/reboot, not live. The lone un-built lever flagged in POWER_LOWPOWER.md
     ...
     * — loses photos with no operator signal (CLAUDE.md: robustness first).
...
void app_config_publish_discovery(void) {
    ...
    static const struct {
         ...
         * on-chip OSD; software decode→re-encode too slow, see NOTES.md).
