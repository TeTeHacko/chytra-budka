...
/* Optional one-time probe per pin — pull-up + pull-down sweep tells
 * us whether *something* is actively driving the line at boot. The
 * verdict seeds likely_present so the first selftest doesn't say
 * pir:false on a wired board. */
static bool pir_probe_driven(int pin) {
    ...
    bool floating = (level_up == 1) && (level_dn == 0);
    ESP_LOGI(TAG, "probe GPIO%d: pull-up=%d pull-dn=%d → %s",
             pin, level_up, level_dn,
             floating ? "no sensor (or AM312 warm-up)" : "driven (sensor present)");
    return !floating;
}
...
esp_err_t pir_init(void) {
    ...
    for (size_t i = 0; i < n_pins; i++) {
        ...
        bool driven = pir_probe_driven(pins[i]);
        atomic_store(&s_inst[i].likely_present, driven);
