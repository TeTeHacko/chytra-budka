...
// TLS trust: sdkconfig builds the mbedTLS bundle from tools/le_isrg_roots.pem
// ONLY (CONFIG_MBEDTLS_CUSTOM_CERTIFICATE_BUNDLE_PATH) — outbound TLS (OTA,
// crash reporter, HTTPS enrollment) is effectively PINNED to the Let's Encrypt
// ISRG X1/X2 roots; saves ~130 bundled roots of flash. If your endpoints don't
// use LE certs, put your CA roots into that PEM instead (or drop the custom
// bundle for the full set). The TLS layer only authenticates the host — image
// integrity is the RSA-3072 app signature (verify-on-update). See ota.c.
