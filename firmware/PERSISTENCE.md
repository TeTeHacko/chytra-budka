...
This page consolidates the **persistence boundary** — which state lives in
which layer ([partitions.csv](partitions.csv), NVS, RTC memory, broker, SD) —
so a fix that relies on state surviving a reset doesn't accidentally rely on a
layer that doesn't actually persist.
...
the SA-Query and associate fine. Beware that some cheap USB WiFi dongles (e.g.
RTL8188EUS-based) have a broken PMF implementation — they advertise PMF but
never answer the SA-Query, so the AP disassociates them (reason 209) before
DHCP. Test the portal with a phone, not one of those.
