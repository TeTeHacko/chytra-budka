...
- `test_csr.c` — CSR build (links the host system mbedTLS, not the IDF target's mbedTLS 4.0.0)
...
## Integration run (against a running relay)
...
all four FW scenarios + Prometheus assertions), [`relay/`](../../../relay/)
ships a `test-e2e.sh` that builds this runner and drives it for you
