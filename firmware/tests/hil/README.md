...
is caught here, not in the field (see `32ec19a`, which shipped on a bench
boot-check alone and `task_wdt`'d on the field's STA path).
...
`CB_PROVISION_SSID` + `CB_PROVISION_PSK` already being exported — and those live
behind `~/.bashrc`'s interactive guard, so they resolve to **empty** in any
script, CI step, or agent shell. The suite then skips provisioning, leaves the
board sitting in its SoftAP, and every later test fails with `no route to host`:
a missing variable that reads exactly like dead hardware. That has burned real
...
## How this slots into the QA cadence

Per the plan in `/home/user/.claude/plans/`:

- **L0 pre-commit**: `make -C firmware/tests/native test` only — no HIL.
- **L1 on push**: still no HIL — host-only, runs against Gitea runners
  without ESP access.
- **L2 nightly**: HIL pytest from this directory, run on the dev host
  where the bench is plugged in (via a Gitea self-hosted runner with
  Docker + USB pass-through, see Phase E in the plan).
- **L3 weekly**: soak + power profiling (Phases C + D, separate dirs).
