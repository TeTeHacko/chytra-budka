...
def bench_port() -> str:
    ...
    if not os.path.islink(port):
        pytest.fail(
            ...
            "in host/README.md; raw /dev/ttyACMn shuffles across replug."
        )
...
def reset_board(bench_id, mqtt_creds):
    ...
    # Guard BEFORE the wipe, not after. This fixture is autouse + session-scoped,
    # so it factory-resets the bench before any test — and provisioned_sta only
    # discovers that the re-provisioning credentials are missing afterwards, by
    # which point the board is already stranded in its SoftAP and every later
    # test fails with "no route to host": a missing variable that reads exactly
    # like dead hardware. The creds live in the operator's ~/.bashrc behind a
    # keyring, so they are EMPTY in any non-interactive shell (a script, a CI
    # step, an agent) — and `pytest` invoked directly is precisely that case.
    # run.sh sources them from NetworkManager, which is why it is the documented
    # entry point. Refuse to start rather than wipe a board we cannot restore.
    if not os.environ.get("CB_PROVISION_SSID") or not os.environ.get("CB_PROVISION_PSK"):
        pytest.exit(
            "HIL refuses to start: this session factory-resets the bench, but "
            "CB_PROVISION_SSID / CB_PROVISION_PSK are not set, so it could not "
            "put the board back on the station LAN afterwards.\n"
            "  Run ./run.sh (it takes them from NetworkManager) instead of bare "
            "pytest,\n"
            "  or export both yourself,\n"
            "  or set CB_HIL_NO_RESET=1 to run against a board you don't want "
            "wiped.",
            returncode=2,
        )
