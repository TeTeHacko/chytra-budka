...
> **Mic note.** Earlier revisions of this README mentioned an external
> INMP441 I²S mic. That was dropped before rev3.1 — the Sense
> expansion's onboard PDM mic conflicts with the SDIO microSD slot if
> we re-used those pins. All current revs run the onboard PDM at
> 16 kHz (stable per Seeed wiki; 48 kHz had partial DMA fills).
...
# LE root bundle — REQUIRED by the build: sdkconfig points the mbedTLS
# certificate bundle at tools/le_isrg_roots.pem (gitignored; safe to re-run).
firmware/tools/fetch_le_roots.sh

# Build via the profile wrapper (it applies the sdkconfig overlay chain —
# a bare `idf.py build` defaults to 2 MB flash and dies at the partition table)
...
pytest                     # against a powered-on bench board
pytest -m "not manual"     # skip mark-manual interaction tests (default anyway)
```
...
(TLS via the **full** `esp_crt_bundle`, NOT pinned — `tools/le_isrg_roots.pem`
exists for optional ISRG-X1/X2 pinning but is not currently wired in; the
RSA-3072 **image signature** is the real integrity control, so a host-CA swap
can't break OTA), compares `esp_app_desc.version` + `app_elf_sha256` against the
running image, and applies the update + restart on mismatch.
