...
| 5V   | —      | input from USB-C / WaveShare      | not used for peripherals |
| GND  | —      | common ground                     | —         |
...
```
After MQTT discovery, `sensor.chytra_budka_<mac>_temperature` and `_humidity`
...
### The AliExpress clones in this batch are broken (2026-05-24)

Two independently bought MAX17048 modules (on two separate boards) behave
identically:

- **Phantom storm**: an `/i2c` scan returns a rotating set of addresses across
  the whole space (0x12, 0x21, 0x36, 0x60, 0x6d…) — different every time, ~5–10 %
  ACK rate on *any* address. The classic "MAX17048 ACKs every address" silicon
  bug (see EEVblog).
- **0x36 is not special**: it shows up in scans as often as the random ghosts. A
  real MAX17048 would ACK on 0x36 in *every* scan as a baseline, with ghosts on
  top. Ours don't.
- **Register reads fail 100 %**: `i2c_master_transmit_receive` (ADDR+W + REG +
  ReSTART + ADDR+R) over 30× retries with `i2c_master_bus_reset()` between
  attempts and a probe warm-up loop got not a single ACK in 300 attempts/boot.
- **Power OK**: `VCC→GND = 3.28 V`, the module's green LED is lit. Same behaviour
  with and without USB (18650 only). Swapping the module and adding external
  4.7 kΩ pull-ups changed nothing.

The driver in `firmware/main/battery.c` has an aggressive probe-then-read retry
strategy (it's sound — it'll help once silicon that actually ACKs on 0x36
arrives). Selftest then reports `battery:false` and the ModeFsm simply skips
that signalling (USB-only operation is a first-class scenario).

**Way forward**: buy a genuine Adafruit or Pimoroni MAX17048, or move to a
BQ27441 or a voltage-divider + ADC fallback. Backlog item B002.
...
**Test after connecting and resetting:**
```
I (xxx) pir: probe GPIO2: pull-up=0 pull-dn=0 → driven (sensor present)
I (xxx) pir: armed on GPIO2 (poll 20 ms × 2 samples = 40 ms debounce) — initial: driven
```
(If the probe says `no sensor (or AM312 warm-up)`, OUT isn't connected or the
module hasn't finished its 5–60 s warm-up. The polling task runs regardless of
the probe verdict — once the warmed-up AM312 starts pulsing, events are counted.
Wave your hand in front of the dome — the firmware requires 3 stable LOW→HIGH
transitions before `pir_ready()` returns `true`, so a single EMI glitch isn't
enough.)
...
| `IN+`        | —        | + from the solar panel (before WaveShare) |
| `IN-`        | —        | + WaveShare solar IN (after the shunt) |
...
## TODO — deployment-only wiring

Wiring for the solar chain and enclosure (WaveShare PMM, panel, antenna + SMA,
IP65 box, gland, mounting, heatsink) isn't documented here yet — it'll be added
once those parts are on the bench. For the parts list, see [SHOPPING.md](SHOPPING.md).
...
- [SCHEMATIC.md](SCHEMATIC.md) — block diagram + power path
