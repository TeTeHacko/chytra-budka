...
# Build the toolbox in its own statement, not inline in the `docker run`
# argument list: `set -e` does not abort on a failed command substitution used
# as an argument, so an inline `$(docker build -q ...)` that fails expands to
# the empty string and `docker run` dies with a misleading
# "invalid reference format" long after the real error has scrolled past.
if ! TOOLBOX="$(docker build -q ./toolbox)" || [ -z "$TOOLBOX" ]; then
  echo "dev-pki: toolbox image build failed (real error is above) — no PKI written" >&2
  exit 1
fi

docker run --rm -v "$ABS_SECRETS:/out" -w /out "$TOOLBOX" sh -eu -c '
    umask 077
