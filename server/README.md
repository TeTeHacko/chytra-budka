...
**LAN-only vs public:** Let's Encrypt needs a publicly resolvable domain with
port 80 reachable from the internet (HTTP-01) — on a LAN the stack simply
stays on the self-signed placeholder cert. That is fine for the browser
(accept it once) and for device **mTLS to the broker** (its trust anchor is
the custom sub-CA, not LE). The device's **HTTPS side** (enrollment POST, OTA
poll) is different: stock field firmware verifies against the Let's Encrypt
roots only, so a LAN-only setup either flashes the `bench` profile
(`CHYTRA_BUDKA_TLS_INSECURE` skips outbound verification — bench only, logged
loudly) or rebuilds with its own CA roots in
`firmware/tools/le_isrg_roots.pem`. Real field devices ⇒ public domain + LE
(the production steps below).
...
2. Provision `BUDKA_SECRETS_DIR` on the host: run `init-secrets.sh`
   ...
   then the PKI chain: `cat root_ca.pem sub_ca_budka.pem > ca_chain.pem`.
   No PKI yet? `./scripts/dev-pki.sh` generates the pair — despite the dev
   name it is a plain 10-year root+sub hierarchy, production-fine if you move
   the root key offline afterwards; swap in your own PKI if you run one.
   Bake the same sub-CA into the firmware as its trust anchor
   (`cp .../sub_ca_budka.pem firmware/main/budka_sub_ca.pem` before building —
   the repo ships a placeholder there).
