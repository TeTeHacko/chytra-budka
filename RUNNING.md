...
- `firmware/main/secrets.h` filled in (copy from `secrets.h.example`) and
  ...
  board boots into an AP provisioning portal, SSID `cb-<suffix>`, with a
  random first-boot password echoed on the serial console / OLED QR; join it
  and submit your network at `http://172.31.4.1/wifi`.)
...
6. **AM312 PIR**, then **IR LED + MOSFET**.
...
- Field deployment (site, climate, power budget): [DEPLOYMENT.md](DEPLOYMENT.md)
